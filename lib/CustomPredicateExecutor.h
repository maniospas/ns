...
class CustomPredicateExecutor: public Object {
    ...
    public:
        const std::string type() const {return "call";};
        int priority = 0;
        ...
        virtual std::shared_ptr<Scope> evaluate_all_arguments(std::shared_ptr<Scope> scope, std::shared_ptr<Expression> predicate);
        virtual std::shared_ptr<Object> call(std::shared_ptr<Scope> scope, std::shared_ptr<Expression> predicate, std::shared_ptr<Scope> derived_scope);
        virtual std::shared_ptr<Object> implement(std::shared_ptr<Scope> scope) = 0;
        static std::shared_ptr<Scope> predicateScope;
        virtual bool can_call(std::shared_ptr<Scope> scope, std::shared_ptr<Expression> predicate, std::shared_ptr<Scope> derived_scope) ;
};
